/**
 * Vanilla JS port of React Bits "MagicRings" (same shaders / behaviour as the React component).
 * Depends on `three` (ES module), e.g. via import map.
 */
 ...
 */
export function mountMagicRings(mount, options = {}) {
  const p = { ...defaultOptions, ...options };
  ...
  if (current.blur > 0) {
    mount.style.filter = `blur(${current.blur}px)`;
  }
  ...
  const animate = (t) => {
    ...
    uniforms.uBurst.value = props.clickBurst ? burstRef : 0;
  ...
  return {
    ...
    /** @param {Partial<typeof defaultOptions>} next */
    setProps(next) {
...
export { defaultOptions as magicRingsDefaults };
